...
def main(args, data, exp=None):
    ...
    # Identity matrix
    E = torch.eye(syn_feat_num, dtype=torch.float32, device=device)
    # Synthetic probabilistic matrix
    edge_probability = data.edge_probability.to(device)
    edge_probability = edge_probability / edge_probability.sum(dim=1)
    P = torch.zeros([syn_feat_num, syn_feat_num], dtype=torch.float32, device=device)
    P = edge_probability[syn_label][:, syn_label]
    Q = E

    print("syn_feat shape: ", syn_feat.shape)
    print("syn_adj shape: ", P.shape)
    ...
    expert_dir = os.path.join(args.buffer_path, args.dataset, args.expert_net)
    ...
    for epoch in range(args.epochs + 1):
        if epoch <= args.epochs:
            ...
            syn_adj = get_syn_adj(syn_feat, P, args.alpha, args.beta, E, Q)
            syn_adj = (torch.abs(syn_adj) + torch.abs(syn_adj.T)) / 2
            ...
            P = args.tau * P + syn_adj.detach() * (1 - args.tau)
            Q = args.gamma * Q + syn_adj.detach() * (1 - args.gamma)
        ...
        if args.test:
            ...
            if epoch in eval_epochs:
                ...
                syn_adj_test = get_syn_adj(syn_feat.detach(), P, args.alpha, args.beta, E, Q)
                syn_adj_test = (torch.abs(syn_adj_test) + torch.abs(syn_adj_test.T)) / 2
    ...
    for _ in range(args.runs_test):
        ...
        res.append(train_with_eval(args.dataset, args.test_net_type, net, syn_feat_test, syn_adj_test, syn_label_test, data, device, 
                                   args.lr_model, args.weight_decay, train_iters=600, multi_label=False, 
                                   normalize=args.normalize, 
                                   verbose=False, with_val=args.with_val))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--alpha', type=float, default=1, help='trade-off')
    parser.add_argument('--beta', type=float, default=1, help='trade-off')
    parser.add_argument('--tau', type=float, default=0.999, help='update P')
    parser.add_argument('--gamma', type=float, default=0.5, help='update Q')
    ...
    parser.add_argument('--with_val', action="store_true", help='cross validation while training gnn')
